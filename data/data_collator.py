...
class DataCollator:
    ...
    def decoder_call(self, batch, return_tensors):
        ...
        self.actual_pad_len = actual_pad_len
...
@dataclass
class DistillAttnDataCollator(DataCollator):
    def decoder_call(self, batch, return_tensors):
        model_inputs = super().decoder_call(batch, return_tensors)
        vocab_size = self.tokenizer.vocab_size
        teacher_logits_list = []
        teacher_attns_list = []
        # make
        for idx, instance in enumerate(batch):
            teacher_logits = instance['teacher_logits']
            label_len = self.label_lens[idx]
            assert teacher_logits.shape[-1] == vocab_size, f"Unmatched vocab size {vocab_size} vs. {teacher_logits.shape[-1]}"
            # pad or cut off the seq dim to actual_pad_len
            if teacher_logits.shape[0] < self.actual_pad_len:
                pad_len = self.actual_pad_len - teacher_logits.shape[0]
                teacher_logits = torch.cat((torch.zeros((pad_len, vocab_size)), teacher_logits), dim=0)
            else:
                teacher_logits = teacher_logits[-self.actual_pad_len:, :]
            # mask the non-label token logits to -100
            teacher_logits[:self.actual_pad_len-label_len, :] = -100.
            # mask gt labels for replay samples
            teacher_logits_list.append(teacher_logits)
            teacher_attns_list.append(instance['teacher_attns'])
        model_inputs['teacher_logits'] = torch.stack(teacher_logits_list).contiguous()
        model_inputs['teacher_attns'] = teacher_attns_list  # bs, n_layers, n_heads, seq_len, seq_len

        return model_inputs


@dataclass
class DistillAttn2DataCollator(DistillAttnDataCollator):
    def decoder_call(self, batch, return_tensors):
        model_inputs = super().decoder_call(batch, return_tensors)
        attn_query = []
        select_heads = []
        for instance in batch:
            attn_query.append(instance['attn_query'])
            select_heads.append(instance['select_heads'])
        # model_inputs['attn_query'] = torch.stack(attn_query)
        model_inputs['attn_query'] = attn_query
        model_inputs['select_heads'] = select_heads
        return model_inputs
