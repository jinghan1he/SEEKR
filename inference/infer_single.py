...
def main():
    ...
    for round in range(task_num):   # load models and adapters of a new round in continual learning
        ...
        for inference_task_id in range(round+1):    # evaluation for previous tasks in a single round
            ...
            if os.path.exists(args.inference_output_path + "/results-" + str(round) + "-" + str(inference_task_id) + "-" + inference_task + ".json"):
                continue
            ...
            if inference_task in ["Py150", "MeetingBank", "task181"]:
                batch_size = 4
            ...
            sources_sequences, predicted_sequences, ground_truths = prediction(args, model, infer_dataloader, tokenizer, device)
            ...
            save_inference_results(args, evaluation_result, sources_sequences, predicted_sequences, ground_truths, round, inference_task_id, inference_task)
